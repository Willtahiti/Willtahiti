...
- [x] Écran web client : afficher les tâches verrouillées (non modifiables) et le formulaire d'ajout libre — `apps/web/app/client/taches/page.tsx` (+ `apps/web/lib/taches.ts`), lien depuis l'écran connecté client. Build/lint/typecheck OK, rendu vérifié (état "non configuré" faute de credentials dans ce sandbox)
- [x] Écran mobile aide-ménagère : afficher la liste complète des tâches d'un client (verrouillées + libres), avec statut — `apps/mobile/components/TachesAideMenagere.tsx` (+ `apps/mobile/lib/taches.ts`), branché via `renderLoggedIn` sur `AuthForm`/`App.tsx`. Typecheck OK ; rendu non vérifié visuellement (réseau sandbox bloqué, comme les tâches précédentes côté mobile)
...
- [x] Test manuel bout en bout : créer un client avec contrat, ajouter une tâche libre, vérifier que l'aide-ménagère voit bien les deux types — simulé de bout en bout sur Postgres 16 local (inscription → seed contrat → ajout tâche libre par le client via RLS → l'aide-ménagère voit les 4 tâches (3 verrouillées + 1 libre) → marque la libre comme faite). À reconfirmer manuellement via les vraies interfaces web/mobile une fois un projet Supabase accessible (réseau sandbox bloqué)
